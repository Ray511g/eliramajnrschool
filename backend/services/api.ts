// API service layer — calls the Next.js API routes on the same server
// When running on Next.js, the API is on the same port (no CORS needed)
const BASE_URL = typeof window !== 'undefined'
    ? '/api'  // Client-side: relative URL (same port)
    : 'http://localhost:3001/api'; // Server-side: absolute URL
