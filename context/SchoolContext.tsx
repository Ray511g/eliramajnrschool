...
export function SchoolProvider({ children }: { children: ReactNode }) {
    ...
    const updateTimetable = async (entries: TimetableEntry[]) => {
        const apiRes = await tryApi(`${API_URL}/timetable/bulk`, { method: 'POST', body: JSON.stringify(entries) });
        if (apiRes) {
            setTimetable(prev => {
                const grades = Array.from(new Set(entries.map(e => e.grade)));
                const filtered = prev.filter(e => !grades.includes(e.grade));
                return [...filtered, ...entries];
            });
            showToast('Timetable updated successfully', 'success');
        } else {
            showToast('Failed to save generated timetable', 'error');
        }
    };
